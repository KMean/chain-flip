 ...
 *
 */
contract ChainFlip is VRFConsumerBaseV2Plus, AutomationCompatibleInterface, ReentrancyGuard {
    /*//////////////////////////////////////////////////////////////
                            TYPE DECLARATIONS
    //////////////////////////////////////////////////////////////*/
    ...
    uint256 private s_minimumBetAmount;
    ...
    mapping(address => uint256) public playerWins; // Track wins for each player easy to retrieve from frontend
    mapping(address => bool) public isPlayer; // Check if an address is already recorded as a player
    mapping(address => uint256) public totalAmountWon;
    mapping(address => uint256) public totalAmountInvested;
    address[] public players; // List of all unique players
    ...
    error CoinFlip__InvalidMinimumBetAmount();
    ...
    error CoinFlip__FeeTooLow();
    error CoinFlip__NoRefundToClaim();
    error CoinFlip__NotValidTimeOut();
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata _randomWords) internal override nonReentrant {
        ...
        playerWins[_match.winner] += 1;
        totalAmountWon[_match.winner] += prizeAmount;
    ...
    function performUpkeep(bytes calldata performData) external nonReentrant {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (_match.state == MatchState.FLIPPING_COIN) {
                ...
                emit MatchCanceledByUpkeep(matchId, _match.vrfRequestId);
            }
        }
    }
    ...
    function createMatch(bool choice) external payable {
        ...
        //match can only increment so it's safe to use unchecked for a uint256
        unchecked {
            s_currentMatchId++;
        }
        ...
        totalAmountInvested[msg.sender] += msg.value;
    ...
    function cancelMatch(uint256 matchId) external {
        ...
        totalAmountInvested[matchToCancel.player1] -= matchToCancel.betAmount;
        emit MatchCanceledByPlayer(matchId, msg.sender);
    }
    ...
    function setFeePercent(uint256 newFee) external onlyOwner returns (uint256) {
        ...
        if (newFee < 1) revert CoinFlip__FeeTooLow();
        feePercent = newFee;
    ...
    function setMinimumBetAmount(uint256 amount) external onlyOwner {
        if (amount < 0.01 ether) revert CoinFlip__InvalidMinimumBetAmount();
    ...
    function setTimeOutForStuckMatches(uint256 timeOut) external onlyOwner {
        if (timeOut < 60) revert CoinFlip__NotValidTimeOut();
    ...
    /**
     * @notice Retrieves all active matches currently in progress.
     * @dev Iterates through the activeMatches array and returns the corresponding Match structs.
     * @return activeMatchesList An array of Match structs representing active matches.
     */
    function getActiveMatches() external view returns (Match[] memory) {
        uint256 activeCount = activeMatches.length;
        Match[] memory activeMatchesList = new Match[](activeCount);

        for (uint256 i = 0; i < activeCount; i++) {
            activeMatchesList[i] = matches[activeMatches[i]];
        }
    ...
    /**
     * @notice Fetches key statistics for a specific player.
     * @param player The address of the player whose statistics are being queried.
     * @return totalMatches The total number of matches the player has participated in.
     * @return totalWins The total number of matches the player has won.
     * @return totalLosses The total number of matches the player has lost.
     * @return totalCanceled The total number of matches the player has canceled.
     * @return totalAmountWonByPlayer The total amount won by the player.
     * @return totalAmountInvestedByPlayer The total amount the player has bet.
     */
    function getPlayerStats(address player)
        ...
        returns (
            ...
            uint256 totalAmountWonByPlayer,
            uint256 totalAmountInvestedByPlayer
        )
    {
        totalMatches = playerMatches[player].length;
        totalWins = playerWins[player];

        uint256 canceledMatches = 0;

        for (uint256 i = 0; i < totalMatches; i++) {
            uint256 matchId = playerMatches[player][i];
            if (matches[matchId].state == MatchState.CANCELED) {
                canceledMatches++;
            }
        }

        totalLosses = totalMatches > totalWins ? totalMatches - totalWins - canceledMatches : 0;
        totalCanceled = canceledMatches;

        totalAmountWonByPlayer = totalAmountWon[player];
        totalAmountInvestedByPlayer = totalAmountInvested[player];

        return
            (totalMatches, totalWins, totalLosses, totalCanceled, totalAmountWonByPlayer, totalAmountInvestedByPlayer);
    }

    /**
     * @notice Retrieves the top N players with the most wins.
     * @dev Sorting can be handled on the frontend for better efficiency.
     * @return allPlayers An array of addresses representing all players.
     * @return allWins An array of win counts corresponding to each player.
     */
    function getTopWinners() external view returns (address[] memory allPlayers, uint256[] memory allWins) {
        uint256 totalPlayers = players.length;
        allPlayers = new address[](totalPlayers);
        allWins = new uint256[](totalPlayers);

        for (uint256 i = 0; i < totalPlayers; i++) {
            allPlayers[i] = players[i];
            allWins[i] = playerWins[players[i]];
        }

        return (allPlayers, allWins);
    }

    /**
     * @notice Fetches matches with pagination support for efficient data retrieval.
     * @dev Useful for UIs that display large datasets incrementally.
     * @param offset The starting index of matches to fetch.
     * @param limit The maximum number of matches to retrieve.
     * @return paginatedMatches An array of Match structs from the specified offset and limit.
     */
    function getMatchesPaginated(uint256 offset, uint256 limit) external view returns (Match[] memory) {
        uint256 totalMatches = s_currentMatchId;
        uint256 fetchCount = limit;

        if (offset + limit > totalMatches) {
            fetchCount = totalMatches - offset;
        }

        Match[] memory paginatedMatches = new Match[](fetchCount);

        for (uint256 i = 0; i < fetchCount; i++) {
            paginatedMatches[i] = matches[offset + i + 1]; // Matches start from ID 1
        }

        return paginatedMatches;
    }

    function getTotalAmountWon(address player) external view returns (uint256) {
        return totalAmountWon[player];
    }

    function getTotalAmountInvested(address player) external view returns (uint256) {
        return totalAmountInvested[player];
    }

    function getTotalWinnings() external view returns (uint256) {
        uint256 totalWinnings = 0;
        for (uint256 i = 0; i < players.length; i++) {
            totalWinnings += totalAmountWon[players[i]];
        }
        return totalWinnings;
    }

    function getTimeOutForStuckMatches() external view returns (uint256) {
        return timeOutForStuckMatches;
    }
}
