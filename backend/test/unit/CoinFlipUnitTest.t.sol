...
contract CoinFlipUnitTest is CodeConstants, Test {
    ...
    function testSetFeePercentTooHigh() public {
        vm.prank(account);
        vm.expectRevert(CoinFlip.CoinFlip__FeeTooHigh.selector);
    ...
    function testCannotCancelCompletedMatch() public createAndJoinMatch {
        vm.prank(PLAYER1);
        vm.expectRevert(CoinFlip.CoinFlip__InvalidMatchState.selector);
        coinflip.cancelMatch(1);
    }

    function testCannotCancelMatchNotCreatedByCaller() public createMatch {
        vm.prank(PLAYER2);
        vm.expectRevert(CoinFlip.CoinFlip__NotTheMatchCreator.selector);
        coinflip.cancelMatch(1);
    }
    ...
    function testSetMinimumBetAmount() public {
        ...
        assertEq(coinflip.getBetAmount(), newAmount);
    }
    ...
    function testTransferFailAndClaimPrizes() public createMatch {
        ...
        emit CoinFlip.PrizeClaimed(address(rejectingWinner), minimumBetAmount * 2 * 95 / 100);
    ...
    function testRetryFailedTransfer() public createMatch {
        ...
        emit CoinFlip.PrizeClaimed(address(rejectingWinner), totalPrize);

        vm.expectEmit(true, true, false, false);
        emit CoinFlip.PrizeTransferRetried(address(rejectingWinner), totalPrize);
    ...
    function testGetMatchState() public createMatch {
        CoinFlip.MatchState state = coinflip.getMatchState(1);
        assertEq(uint256(state), uint256(CoinFlip.MatchState.WAITING_FOR_PLAYER));
        ...
        assertEq(uint256(state), uint256(CoinFlip.MatchState.FLIPPING_COIN));
        ...
        state = coinflip.getMatchState(1);
        assertEq(uint256(state), uint256(CoinFlip.MatchState.ENDED));
    }
    ...
    function testGetRefunds() public createMatch {
        ...
        vm.prank(PLAYER1);
        coinflip.cancelMatch(1);
        assertEq(coinflip.getRefunds(PLAYER1), coinflip.getBetAmount());
    }
    ...
    function testMatchJoinedEmitsEvent() public createMatch {
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.MatchJoined(1, PLAYER2, false, minimumBetAmount);
    ...
    function testMatchCanceledEmitsEvent() public createMatch {
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.MatchCanceled(1, PLAYER1);
    ...
    function testMatchEndedEmitsEvent() public createAndJoinMatch {
        ...
        vm.expectEmit(true, true, false, false);
        emit CoinFlip.TransferPrize(PLAYER2, prizeAmount);
        vm.expectEmit(true, true, true, false);
        emit CoinFlip.MatchResult(1, PLAYER2, 1);
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.MatchEnded(1, PLAYER2, (2 * minimumBetAmount) - fee, block.timestamp);
        VRFCoordinatorV2_5Mock(vrfCoordinator).fulfillRandomWordsWithOverride(requestId, address(coinflip), randomWords);
    }
    ...
    function testRefundIssuedEmitsEvent() public createMatch {
        ...
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.RefundIssued(PLAYER1, minimumBetAmount);
    ...
    function testRefundIssuedFailedEmitsEvent() public {
        ...
        vm.expectRevert(CoinFlip.CoinFlip__TransferFailed.selector);
        coinflip.withdrawRefund();
        vm.stopPrank();
    }
    ...
    function testTransferPrizeEmitsEvent() public createAndJoinMatch {
        ...
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.TransferPrize(PLAYER2, (2 * minimumBetAmount) - fees);
    ...
    function testMinimumBetAmountUpdatedEmitsEvent() public {
        vm.startPrank(account);
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.MinimumBetAmountUpdated(0.02 ether);
    ...
    function testFeesWithdrawnEmitsEvent() public createAndJoinMatch {
        ...
        vm.expectEmit(true, true, true, true);
        emit CoinFlip.FeesWithdrawn(DEVADDRESS, collectedFees);
...
contract RejectingWinner {
    ...
    constructor(address _coinflip) {
        coinflip = CoinFlip(payable(_coinflip));
    }
    ...
    // Function to manually call claimPrize() in CoinFlip
